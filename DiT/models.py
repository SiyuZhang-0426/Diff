...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(hidden_size, 6 * hidden_size, bias=True)  # all the affine parameters are learned by a single mlp
        )
...
class DiT(nn.Module):
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        # For exact reproducibility reasons, we apply classifier-free guidance on all input channels.
